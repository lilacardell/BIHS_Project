...
Siyao's part
1.1 Select y8 data (health worker visits)
...
1.2 select I1 data (Summary of Agriculture Production)

#This needs to be calculated/aggregated by household and should not be merged until then 
#Do not change $ variables (NA to 0)
```{r}
...
# code NA values to 0 (on quantity, reasons, etc.) do not to for $$$
#Ag_production[is.na(Ag_production)] = 0

# rename variables (we don't need these)
#Ag_production = Ag_production%>%
 # rename(hhid = a01, crop_code = crop, qty_harvested_kg = i1_01, qty_received_shareOutPlot_kg = i1_02, qty_land_owner_shareOutPlot_kg = i1_03, qty_irrigation_purpose_kg = i1_04, qty_gave_to_labor_kg = i1_05, qty_for_consumed_kg = i1_06, qty_donationOthers_kg = i1_07,qty_animalFeed_kg = i1_08, qty_damage_kg = i1_09a, reason_damage = i1_09b, qty_sold_kg = i1_10, where_sold = i1_11, distance_salePoint_km = i1_12a, time_trave_min = i1_12b, sale_taka = i1_13, qty_store_seed_kg = i1_14, current_stock_kg = i1_15)

```

1.3 select I2 data (Food Grain Stock and Storage Capacity)

```{r}
#grain_stock = %>%
  #melt(setDT(x1_f_2011_m),measure.vars=patterns("icode","iweight"),variable.name="inumber",value.name=c("icode","iweight"))%>%
# code `without stocks` of `End of Month food grain stock paddy, December 2013 (kg)` to zero
#grain_stock$i2_02_1 = ifelse((grain_stock$i2_02_1 == 99999), 0, grain_stock$i2_02_1)
# rename variables
...
grain_stock = i2_male_2011%>%
  pivot_longer(cols = i2_02_1:i2_13_3, names_to = 'grain_month', values_to = 'qty')

# grain_stock$grain_type = ifelse(str_detect(grain_stock$grain_month, fixed("02")),"Dec2010",
                                
                         # ifelse(str_detect(grain_stock$grain_month, fixed("03")),"Jan2011",
                         # ifelse(str_detect(grain_stock$grain_month, fixed("04")),"Feb2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("05")),"Mar2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("06")),"Apr2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("07")),"May2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("08")),"June2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("09")),"July2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("10")),"Aug2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("11")),"Sep2011", 
                         # ifelse(str_detect(grain_stock$grain_month, fixed("12")),"Oct2011", 
                         # "Nov2011"))

grain_stock = grain_stock%>%
  pivot_wider(names_from = grain_type, values_from = qty)%>%
  select(-grain_month)
...
1.4 select R01/R03 data (Sanitation and Water; R01 = latrine type, R03 = water source)

```{r}
#select r01 and r03 columns and rename variables
latrine_water = r_male_2011%>%
  dplyr::select("a01", contains("r01"), "r06")%>%
  rename(hhid = a01, latrine_type = r01, drinkwater_souce = r06)
...
latrine_water$hh_lact = ifelse(latrine_water$latrine_type %in% c(2,3,4,5), 1,0)
latrine_water$treated_drinkwater = ifelse(latrine_water$drinkwater_souce %in% c(1,2,3,4,6), 1,0)

```

1.5 select U data (Social Safety Net Programs)
#see comments below
```{r}
...
#dplyr::summarise()%>%
  rename(any_assist = u01, cash_received = u02)%>%
  plyr::mutate(inkind_val=rowSums(.[10:12],na.rm=TRUE))%>% #missing rice value and rice and wheat need to be multiplied 
  group_by(hhid)%>%
...
1.6 Merge selected variables with w.HH.roster.2011
```{r}
...
1.7 Coding for table `v1`(Profile of Current Migrants, Domestic and International) :

```{r}
# select all the household having any members who are migrants(condition: v1_01 == 1)
#ibrary(tidyverse)
#library(dplyr)
current_migrants = v1_male_2011%>%filter(v1_01 == 1)%>%
  select(-c(v1_01))%>%
...
current_migrants[current_migrants$sending_remittance == 2, ]$sending_remittance = 0
mean(current_migrants$sending_remittance)
```
...
# Other (specify) ....................................76
current_migrants%>%group_by(education)%>%
...
current_migrants%>%group_by(the_way_migration_expense_paid)%>%
  dplyr::summarise(mean(sending_remittance))
#plot(corr_13_prop_14)
```

1.8 Coding for table `v2`(Remittance in):
...
1.9 Coding for table `v3`(Remittance_out):
...
1.10 Coding for table `v4`(Other Income Household):
...
w.hh.roster.2011 = merge(x =other_income, y = w.hh.roster.2011, by = "hhid")
#nrow(w_hh_v2)
```

1.11 Coding for table `T1`(Negative Shocks):
...
1.12 Coding for table `T2`(Positive Events):
...
1.13 Merge negative and positive events with w.hh.remittance.otherIncome
```{r}
# merge negative and positive events with w.hh.remittance.otherIncome
w.hh.roster.2011 = merge(x =w.hh.roster.2011, y = neg_pos, by = "hhid")
```


1.14 Merge all variables to w.hh.roster.2011 (roster.hh.2011.with.var)
```{r}
...
write.csv(w.I.roster.2011,"./rosters/w.I.roster_2011.csv",row.names=FALSE)

write.csv(w.hh.roster.2011,"./rosters/w.hh.roster_2011.csv",row.names=FALSE)
```
