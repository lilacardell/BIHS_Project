...
knitr::opts_chunk$set(echo = FALSE,root.dir='D:/SP21/BIHSgit/BIHS_Project ',warning = FALSE, message=FALSE)
options(scipen=999)
set.seed(12345)
...
setwd("D:/SP21/BIHSgit/BIHS_Project/Data")
...
setwd("D:/SP21/BIHSgit/BIHS_Project/Data/Clean Data/nutrient tables")
...
#task:  line 165: cwgt does not exist, changed to "x1_09"
...
x1_f_2015_m = left_join(x1_female_2015,menucodes,by=c("menu"))%>%
  ...
  #task: reorder columns of x1_f_2015 : hh_type move to the 3rd from the end
...
roster_2015_I=left_join(b1_male_2015,anthro_2015_I,by=c("a01","mid","sample_type")) %>%
...
villages = aggregate(a01~vcode_n, data=a_male_2015, FUN = length) #318 villages
...
sample_type = aggregate(hhid~sampletype, data=roster_2015_hh, FUN = length)
#1:FTF Original 1000
#2:FTF Additional 1080
#3:National Representative 4423

#import weights
weights_2015 <- read_dta("D:/SP21/BIHSgit/BIHS_Project/Data/Raw Data/bihs_2015_2012/IFPRI_BIHS_R1_expenditure.dta") %>%
  dplyr::select(a01,dvcode,vcode,aeu,hhweightR1,popweightR1)

#weights_2015 <- read_dta("D:/SP21/BIHSgit/BIHS_Project/Data/Raw Data/bihs_2015/IFPRI_BIHS_R2_exp.dta")
...
design_HH <- svydesign(data=w.hh.roster.2015,id=~vcode, strata=~dvcode,weights=~hhweightR1)
design_I <- svydesign(data=w.I.roster.2015,id=~vcode, strata=~dvcode,weights=~popweightR1)
...
setwd("D:/SP21/BIHSgit/BIHS_Project/Programs/R/output")
...
AE_India <-read_excel("D:/SP21/BIHSgit/BIHS_Project/Data/Clean Data/nutrient standards/GuidelinesIndia_2015.xlsx",sheet="AE_India",range = "A1:D13",col_types =  "numeric")
...
India_RDA<-read_excel("D:/SP21/BIHSgit/BIHS_Project/Data/Clean Data/nutrient standards/GuidelinesIndia_2015.xlsx",sheet="India_RDA",range = "A1:k19",col_names = TRUE)%>%
...
setwd("D:/SP21/BIHSgit/BIHS_Project/Data/Clean Data/nutrient standards")

#FAO weight height
hfa_girls_24 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="hfa_girls_25",col_names = TRUE)%>%
...
wfa_girls_24 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="wfa_girls_05",col_names = TRUE)%>%
  mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
...
hfa_girls_519 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="hfa_girls_519",col_names = TRUE)%>%
   mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("h",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("h_P"))%>%
  mutate(h_med=h_P50)

wfa_girls_510 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="wfa_girls_510",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("w",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("w_P"))%>%
  mutate(w_med=w_P50)

bmi_girls_519 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="bmi_girls_519",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
...
hfa_boys_24 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="hfa_boys_25",col_names = TRUE)%>%
...
wfa_boys_24 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="wfa_boys_05",col_names = TRUE)%>%
  mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("w",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("w_P"))%>%
  filter(year%in%(2:4))%>%
...
hfa_boys_519 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="hfa_boys_519",col_names = TRUE)%>%
   mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("h",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("h_P"))%>%
  mutate(h_med=h_P50)

wfa_boys_510 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="wfa_boys_510",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("w",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("w_P"))%>%
  mutate(w_med=w_P50)

bmi_boys_519 <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="bmi_boys_519",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("b",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("b_P"))%>%
  mutate(b_med=b_P50)
...
FAO_adult <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="Adult_BMRPAL",range = "A1:M37",col_names = TRUE)%>%
  dplyr::select(1:6,9:11,13)%>%
  mutate(sex=if_else(sex=="Male",1,2))%>%
  filter(agemin>=18)

FAO_child <- read_excel("./WHOFAO_requirements_2015.xlsx",sheet="Child_TEEPAL",range = "A1:P97",col_names = TRUE)%>% 
  mutate(sex=if_else(sex=="Male",1,2))%>%
...
#DRI standards
#energy requirements
IOM_energy <- read_excel("D:/SP21/BIHSgit/BIHS_Project/Data/Clean Data/nutrient standards/IOM_requirements.xlsx",sheet="Energy_IOM",range = "A1:R97",col_names = TRUE)%>% 
...
IOM_nutrients <- read_excel("D:/SP21/BIHSgit/BIHS_Project/Data/Clean Data/nutrient standards/IOM_requirements.xlsx",sheet="Nutrients",range = "A1:T23",col_names = TRUE)%>% 
#dplyr::select(-9,-11)%>% 
