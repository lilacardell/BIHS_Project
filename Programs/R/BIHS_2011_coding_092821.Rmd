...
recipe_I = x2_female_2011%>%
  ...
  dplyr::select(-sample_type.y,-sample_type.x,-starts_with("sum_"),-menucookwt,-date,-rid,-foodsource,-menuname)

nrow(filter(recipe_I,is.na(i_mealsum_kcal))) #130611 did not match
...
#   mutate(i_mealsum_kcal=if_else(is.na(i_mealsum_kcal),(icookwt/menucookwt)*sum_kcal,i_mealsum_kcal),
#          i_mealsum_protein=if_else(is.na(i_mealsum_protein),(icookwt/menucookwt)*sum_protein,i_mealsum_protein),
#          i_mealsum_fat=if_else(is.na(i_mealsum_fat),(icookwt/menucookwt)*sum_fat,i_mealsum_fat),
#          i_mealsum_carb=if_else(is.na(i_mealsum_carb),(icookwt/menucookwt)*sum_carb,i_mealsum_carb),
#       i_mealsum_calcium=if_else(is.na(i_mealsum_calcium),(icookwt/menucookwt)*sum_calcium,i_mealsum_calcium),
#     i_mealsum_iron=if_else(is.na(i_mealsum_iron),(icookwt/menucookwt)*sum_iron,i_mealsum_iron))%>%
#   dplyr::select(-starts_with("sum_"))
# 
# nrow(filter(recipe_I,is.na(i_mealsum_kcal))) #54110 did not match, better
...
latrine_water = r_male_2011%>%
  ...
  dplyr::rename(hhid = a01, latrine_type = r01, drinkwater_source = r06)%>%
...
save_kable(descriptives,"./Output/2011/BIHS_Projectdescriptives.png")


#a=qflextable(ab)
#write.table(ab,file="./Output/2011/descriptives.doc")
...
mutate(kcal_FAO_sample=if_else(pregnant==1,multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp)+285,
                           if_else(lactating==1,multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp)+485,
                           multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp))),
       AE_FAO_sample=kcal_FAO_sample/base_kcal_FAO,AE_FAO_age=kcal_FAO_age/base_kcal_FAO,AE_FAO_grp=kcal_FAO_grp/base_kcal_FAO,
...
mutate(age_grp_IOM=if_else(!is.na(agemin),paste(as.character(agemin),as.character(agemax),sep="-"),as.character(NA)))%>%
  mutate(kcal_IOM_sample=eer1-(eer2*age)+refPA*((eer3*weight)+(eer4*height/100))+eer5+eer6)%>%
  mutate(AE_IOM_grp=kcal_IOM_grp/base_kcal_IOM,AE_IOM_sample=kcal_IOM_sample/base_kcal_IOM)%>%
...
w.I.roster.2011=w.I.roster.2011%>%
  ...
  dplyr::rename(sex=sex.x,pregnant=pregnant.x,lactating=lactating.x)%>%
    mutate(protein_ear_IOM_grp=protein_ear_perkg*weight_IOM_grp,protein_ear_IOM_sample=protein_ear_perkg*weight)%>%
...
w.I.roster.2011=left_join(w.I.roster.2011,h,by="hhid")%>%
  mutate(share_India_grp=AE_India_grp/hhsize_India_grp,share_FAO_grp=AE_FAO_grp/hhsize_FAO_grp,share_IOM_grp=AE_IOM_grp/hhsize_IOM_grp,share_FAO_sample=AE_FAO_sample/hhsize_FAO_sample,share_IOM_sample=AE_IOM_sample/hhsize_IOM_sample,share_FAO_age=AE_FAO_age/hhsize_FAO_age,
