...
packages  =  c('data.table','tidyverse','haven','stargazer','plm','lubridate','survey','psych','knitr','xtable','tables','kableExtra','fuzzyjoin','MASS','matrixcalc','car','stringr','styler','readxl','magrittr','sjmisc','readxl','srvyr','gdata','labelled','anthro')
...
India_RDA<-read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/GuidelinesIndia_2011.xlsx",sheet="India_RDA",range = "A1:k19",col_names = TRUE)%>%
  ...
  dplyr::select(-1)
#set activity to light for all under 18
...
w.I.roster.2011=w.I.roster.2011%>%
  ...
  fuzzy_left_join(India_RDA,
                ...
                match_fun=list(`==`,`==`,`>=`, `<=`))%>%
   dplyr::rename(activity=activity.x,sex=sex.x,kcal_India_ref=kcal_India)%>%
  mutate(range_India=if_else(!is.na(agemin),paste(as.character(agemin),as.character(agemax),sep="-"),as.character(NA)),
    kcal_India_ref=if_else(pregnant==1,kcal_India_ref+350,if_else(lactating==1,kcal_India_ref+560,kcal_India_ref)),
...
Import FAO height weight and Nutrient Requirements
```{r FAO}

#FAO weight height
hfa_girls_24 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="hfa_girls_25",col_names = TRUE)%>%
...
wfa_girls_24 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="wfa_girls_05",col_names = TRUE)%>%
  mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
...
hfa_girls_519 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="hfa_girls_519",col_names = TRUE)%>%
   mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("h",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("h_P"))%>%
  mutate(h_med=h_P50)

wfa_girls_510 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="wfa_girls_510",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("w",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("w_P"))%>%
  mutate(w_med=w_P50)

bmi_girls_519 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="bmi_girls_519",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=2)%>%
...
hfa_boys_24 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="hfa_boys_25",col_names = TRUE)%>%
...
wfa_boys_24 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="wfa_boys_05",col_names = TRUE)%>%
  mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("w",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("w_P"))%>%
  filter(year%in%(2:4))%>%
...
hfa_boys_519 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="hfa_boys_519",col_names = TRUE)%>%
   mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("h",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("h_P"))%>%
  mutate(h_med=h_P50)

wfa_boys_510 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="wfa_boys_510",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("w",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("w_P"))%>%
  mutate(w_med=w_P50)

bmi_boys_519 <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHO/WHOFAO_requirements_2011.xlsx",sheet="bmi_boys_519",col_names = TRUE)%>%
mutate(year=floor(Month/12),month=Month-year*12,sex=1)%>%
    dplyr::rename_at(.vars=vars(contains("P")),.funs=funs(paste("b",.,sep="_")))%>%
  dplyr::select(sex,year,month,contains("b_P"))%>%
  mutate(b_med=b_P50)
...
FAO_hwb_med=bind_rows(FAO_women_med,FAO_men_med)%>%
  ...
  mutate(h_grp_FAO=median(h_med),w_grp_FAO=median(w_med))%>%
...
FAO_adult <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHOFAO_requirements_2011.xlsx",sheet="Adult_BMRPAL",range = "A1:N37",col_names = TRUE)%>%
  dplyr::select(1:6,9:11,14)%>%
  mutate(sex=if_else(sex=="Male",1,2))%>%
  filter(agemin>=18)

FAO_child <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/WHOFAO_requirements_2011.xlsx",sheet="Child_TEEPAL",range = "A1:Q97",col_names = TRUE)%>% 
  mutate(sex=if_else(sex=="Male",1,2))%>%
dplyr::select(1:6,11,13,14,17)
...
kcal_FAO_I=if_else(pregnant==1,multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp)+285,
                           if_else(lactating==1,multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp)+485,
                           multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp))),
       kcal_FAO_ref=if_else(pregnant==1,EER+285,if_else(lactating==1,EER+485,EER)),
       protein_ear_FAO_ind=if_else(pregnant==1,protein_FAO_perkg*weight+15.75,if_else(lactating==1,protein_FAO_perkg*weight+13.67,protein_FAO_perkg*weight)),
       protein_ear_FAO_ref=if_else(pregnant==1,protein_FAO_perkg*refwt_FAO+15.75,if_else(lactating==1,protein_FAO_perkg*refwt_FAO+13.67,protein_FAO_perkg*refwt_FAO)),

base_FAO=FAO%>%
  filter(agemin==18,sex==1,activity=="moderate")
base_kcal_FAO=base_FAO$EER
...
w.I.roster.2011=w.I.roster.2011%>%
 mutate(sex=as.numeric(sex))%>%
  left_join(FAO_hwb_med,by=c("sex","age","pregnant","lactating"))
...
mutate(range_FAO=if_else(!is.na(agemin),paste(as.character(agemin),as.character(agemax),sep="-"),as.character(NA)),
      kcal_FAO_ind=if_else(pregnant==1,multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp)+285,
                           if_else(lactating==1,multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp)+485,
                           multiplier*(kc+kc1*weight+kc2*(weight^2)+E_disp))),
       kcal_FAO_ref=if_else(pregnant==1,EER+285,if_else(lactating==1,EER+485,EER)),
       protein_ear_FAO_ind=if_else(pregnant==1,protein_FAO_perkg*weight+15.75,if_else(lactating==1,protein_FAO_perkg*weight+13.67,protein_FAO_perkg*weight)),
  ...
  dplyr::select(-contains(".y"),-contains("kc"),-E_disp,-multiplier,-EER,-agemin,-agemax,-protein_FAO_perkg)

#missing=w.I.roster.2011%>%filter(is.na(refwt_FAO))
  AE_FAO_ind=kcal_FAO_ind/base_kcal_FAO
...
dplyr::rename(agemin=age_lower_yrs,agemax=age_upper_yrs,kcal_IOM_ref=EER_IOM_ref)%>%
...
#dplyr::select(-9,-11)%>% 
  ...
  dplyr::rename(agemin=age_lower_yrs,agemax=age_upper_yrs,carb_IOM_lower=carbohydrate_lower,carb_IOM_upper=carbohydrate_upper,protein_IOM_lower=protein_lower,protein_IOM_upper=protein_upper,fat_IOM_lower=lipids_lower,fat_IOM_upper=lipids_upper)%>%
...
base_kcal_IOM=base_IOM$kcal_IOM_ref
...
mutate(range_IOM=if_else(!is.na(agemin),paste(as.character(agemin),as.character(agemax),sep="-"),as.character(NA)))%>%
  mutate(kcal_IOM_ind=eer1-(eer2*age)+refPA*((eer3*weight)+(eer4*height/100))+eer5+eer6)%>%
  mutate(AE_IOM_ref=kcal_IOM_ref/base_kcal_IOM,AE_IOM_ind=kcal_IOM_ind/base_kcal_IOM)%>%
...
w.I.roster.2011=w.I.roster.2011%>%
  ...
  dplyr::rename(sex=sex.x,pregnant=pregnant.x,lactating=lactating.x,calcium_ear_IOM=calcium_ear,iron_ear_IOM=iron_ear,
                carb_ear_IOM=carb_ear)%>%
    mutate(protein_ear_IOM_ref=protein_ear_perkg*refwt_IOM,protein_ear_IOM_ind=protein_ear_perkg*weight)%>%
    dplyr::select(-contains(".y"),-protein_ear_perkg,-agemin,-agemax)

xlist=c("protein","carb","fat")
ylist=c("lower","upper")
zlist=c("ref","ind")
...
for (x in xlist){
  for (y in ylist){
    for (z in zlist){
 w.I.roster.2011[[paste(x,y,"IOM",z,sep="_")]]<-  (w.I.roster.2011[[paste(x,"IOM",y,sep="_")]]/100)*w.I.roster.2011[[paste("kcal_IOM",z,sep="_")]]
...
ref_wtht = w.I.roster.2011%>%
  dplyr::select(hhid,mid,sex,sampletype,contains("age"),contains("range"),pregnant,lactating,weight,contains("refwt"),height,contains("refht"),contains("code"),contains("R1"))
...
w.I.roster.2011=left_join(w.I.roster.2011,h,by="hhid")%>%
  mutate(share_India=AE_India/hhsize_India,share_FAO_ref=AE_FAO_ref/hhsize_FAO_ref,share_IOM_ref=AE_IOM_ref/hhsize_IOM_ref,share_FAO_ind=AE_FAO_ind/hhsize_FAO_ind,share_IOM_ind=AE_IOM_ind/hhsize_IOM_ind,
...
allref=w.I.roster.2011%>%
  ...
  dplyr::select(hhid,mid,age,age_range,sex,weight,contains("wt_"),height,contains("ht_"),contains("AE_"),contains("code"),contains("R1"))
