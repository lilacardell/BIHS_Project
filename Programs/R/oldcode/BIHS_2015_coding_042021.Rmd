...
1.1 select y6a data (access to clinics)
...
1.2 select y8 data (health worker visits)
...
1.3 select I1 data (Summary of Agriculture Production)
...
1.4 select I2 data (Food Grain Stock and Storage Capacity)
...
1.5 select R01/R06a data (Sanitation and Water; R01 = latrine type, R03 = water source)
...
1.6 select U data (Social Safety Net Programs)
...
```
1.7 select v4 data (Other Income Household)
```{r}
...
1.8 select T1 data (Negative Shocks)
```{r}
...
1.9 select T2 data (Positive Events)
```{r}
...
1.10 select C data (Non-ag employment)
```{r}
...
nonag_employment_hh = aggregate(nonag_employment[, 2], by=list(nonag_employment$hhid), FUN=sum, na.rm = TRUE)
  
```

1.11 select D1 & D2 data (assets)
```{r}
...
assets = d1_male_2015%>%
  ...
  rename(hhid = a01, hh_purchase_val=d1_09,hh_curr_val=d1_10,ag_purchase_val=d2_09,ag_curr_val=d1_10)%>%

#create household level income from non-ag employment
assets_hh = aggregate(nonag_employment[, 2], by=list(nonag_employment$hhid), FUN=sum, na.rm = TRUE)
...
1.1？ merge all modules to the roster
```{r}
