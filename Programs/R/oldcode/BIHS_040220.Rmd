...
10. Add HH level variables back to individual roster (not yet?)
```{r}
...
wroster_2011_I=wroster_2011_I%>%
  ...
  dplyr::select(-contains(".y"),weight_kg,-agemin,-agemax)%>%
  mutate(kcal_india=if_else(pregnant==1,kcal_india+250,if_else(lactating==1,kcal_india+560,kcal_india)),
         protein_india=if_else(pregnant==1,protein_india+23,if_else(lactating==1,protein_india+16,protein_india)))

rm(AE_India,India_RDA)


#FAO requirements and AE
#read in FAO AME
AE_FAO <- read_excel("./Clean Data/FAO_energyrequirements_2011.xlsx",sheet="AME_FAO",range = "A1:F22",col_names=TRUE,col_types = "numeric")

#join AE
wroster_2011_I=fuzzy_left_join(wroster_2011_I,AE_FAO, 
                by=c("age"="agemin", "age"="agemax"),
                match_fun=list(`>=`, `<=`))%>%
  mutate(AE_FAO=if_else(sex==1,AME_male,if_else(sex==2&pregnant==0&lactating==0,AME_female,
                                               if_else(sex==2&pregnant==1,AME_pregnant,
                                                       if_else(sex==2&lactating==1,AME_lactating,as.numeric(NA))))))%>%
  dplyr::select(-(49:54))

#read in how to calculcate energy requirements for each individual
FAO_BMR <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/FAO_energyrequirements_2011.xlsx",sheet="BMR_PAL",range = "A1:H13",col_names = TRUE)%>% 
  dplyr::select(1,2,6,7)%>% 
  mutate(sex=if_else(sex=="Male",1,2))%>% 
  dplyr::rename(agegrp_FAO=age)

FAO_PAL <-read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/FAO_energyrequirements_2011.xlsx",sheet="BMR_PAL",range = "M1:P4",col_names = TRUE)
...
nutrition_2011_I=wroster_2011_I%>%
 dplyr::select(1:24,36:40)%>%
mutate(sex=as.numeric(sex),agegrp_FAO=if_else(age>=60,"60-150",if_else(age>=30 & age<60,"30-60",if_else(age>=18 & age<30,"18-30",if_else(age>=10 & age<18,"10-18",if_else(age>=3 & age<10,"3-10",if_else(age>=0 & age<3,"0-3",as.character(NA))))))))%>%
  left_join(FAO_BMR,by=c("agegrp_FAO","sex"))%>%
  mutate(BMR_kcal=(weight*BMR_KCd1)+BMR_KCd2)%>%
  dplyr::select(-BMR_KCd1,-BMR_KCd2)%>%
  left_join(FAO_PAL,by="activity")%>%
  mutate(kcal_FAO_PAL=BMR_kcal*SF_PAL)%>%
  mutate(kcal_FAO_PAL_pl=if_else(pregnant==1,kcal_FAO_PAL+285,if_else(lactating==1,kcal_FAO_PAL+485,kcal_FAO_PAL)))%>%
  dplyr::select(-PAL_value,-midpoint_PAL,-SF_PAL)

kcal_grp=nutrition_2011_I%>%
  group_by(agegrp_FAO,sex)%>%
  dplyr::summarise(kcal_grp=mean(kcal_FAO_PAL_pl,na.rm=TRUE))
...
DRI/WHO standards Schenider Hertforth (2020)
```{r Kate}

#WHO/DRI standards from Schenider/Hertforth
who_standards = read_dta("C:/Users/lilac2/Box/BIHS Project/Programs/Stata/EAR+UL+AMDR+CDDR+BF_WHOgrowthref_allgroups.dta")%>%
mutate(sex=if_else(age_sex_grp%in%c(4,6,8:13),1,if_else(age_sex_grp%in%c(5,7,14:25),2,as.numeric(NA))),
  ...
  filter(nutr_no %in%c(2,3,4,5,16,18))%>%
dplyr::mutate(nutr=revalue(nutr,c("Energy"="kcal","Carbohydrate"="carb","Protein"="protein","Lipids"="fat","Calcium"="calcium","Iron"="iron")))

who_split=split(who_standards,who_standards$nutr)
who_names=names(who_split)
for (i in 1:length(who_names)) {
  y=who_split[[i]]
  assign(paste("who",who_names[i],sep="_"), y)
  x=nutrition_I_LC%>%dplyr::select(1:29,contains(who_names[i]))%>%left_join(y,by=c("agegrp_who","sex"))
  assign(paste("nutrition",who_names[i],sep="_"),x)
}
...
Energy gaps
```{r}
who_kcal=who_standards%>%
  
    mutate(kcal_PAL_pl_gap_hh=(kcal_PAL_pl_hh-hh_kcal)/kcal_PAL_pl_hh,BMI_gap_I=(18.5-BMI)/18.5,kcal_gap_PC_I=(hh_kcal_PC-I_kcal)/hh_kcal_PC, 
          kcal_gap_AE_I=(hh_kcal_AE_hh-I_kcal)/hh_kcal_AE_hh, 
           kcal_PAL_pl_gap_I=(kcal_PAL_pl-I_kcal)/kcal_PAL_pl,
                    kcal_gap_I=(kcal_india-I_kcal)/kcal_india,
           protein_gap_I=(protein_india-I_protein)/protein_india,
           fat_gap_I=(fat_india-I_fat)/fat_india,
           calcium_gap_I=(calcium_india-I_calcium)/calcium_india,
...
# 
# for (x in nutnames){
#   inequality_x=wroster_2011_I_LC%>%
#   group_by(agegrp_FAO,sex)%>%
#   dplyr::summarise([[paste(x,"_gap_I",sep="")]]=mean(energy_gap_I,na.rm=T),cal_gap_I=mean(cal_gap_I,na.rm=T),cal_gap_PC=mean(cal_gap_PC_I,na.rm=T),
...
inequality_bygroup=wroster_2011_I_LC%>%
  group_by(agegrp_FAO,sex)%>%
  dplyr::summarise(kcal_gap_I=mean(kcal_gap_I,na.rm=T),kcal_gap_I=mean(kcal_gap_I,na.rm=T),kcal_gap_PC=mean(kcal_gap_PC_I,na.rm=T),
            kcal_gap_AE=mean(kcal_gap_AE_I,na.rm=T),BMI_gap_I=mean(BMI_gap_I,na.rm=T))

print(inequality_bygroup,digits=2)
```
