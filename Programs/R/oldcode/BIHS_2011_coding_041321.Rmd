...
#current_migrants = v1_male_2011%>%filter(v1_01 == 1)%>%
 # select(-c(v1_01))%>%
  #dplyr::rename(hhid = a01, relation = v1_02, migr_y = v1_03, mig_mo = v1_04, age = v1_05, sex = v1_06, education = v1_07, occupation = v1_08, country_migr = #v1_09,zila_code = v1_10, country_code = v1_11, migr_purpose = v1_12, migr_exp = v1_13, remitway_send = v1_14)
```

```{r}
# recode levels "No" of v1_14 from 2 to 0 
#current_migrants[current_migrants$remitway_send == 2, ]$remitway_send = 0
#mean(current_migrants$remitway_send)
```
...
#current_migrants%>%group_by(education)%>%
 # dplyr::summarise(mean(remitway_send))%>%
 # ungroup()
```
...
#current_migrants%>%group_by(migr_exp)%>%
 # dplyr::summarise(mean(remitway_send))%>%
 # ungroup()
...
#remittance_in = v2_male_2011%>%
 # rename(hhid = a01, remit_receive = v2_01, relation = v2_02,  remitter_zila_code = v2_03, remitter_country_code = v2_04, receiving_time = v2_05, receiving_amt #= v2_06, the_way_remittance_sent = v2_07, expenditure_cut_without_receiving = v2_08, senter_putting_condition = v2_09, items = v2_10)%>%
 # select(-flag)%>%
...
#, shocks = t1_02, times_of_shocks = t1_03, month_when_shocks_happen = t1_04, 
...
#, event = t2_02, experiencing_positive_event = t2_03, the_month_when_positive_events_happen = t2_04, the_year_when_positive_events_happen = #t2_05,value_of_received_items = t2_06, rank_of_positive_effects = t2_07)%>%
...
neg_pos = left_join(neg_shock,pos_shock, by = "hhid")
```

1.14 select y6a data (access to clinics)
...
1.15 Merge all variables to w.hh.roster.2011 (roster.hh.2011.with.var)
```{r}
