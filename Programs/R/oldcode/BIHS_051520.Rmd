...
FAO_adult <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/FAO_energyrequirements_2011.xlsx",sheet="Adult_BMRPAL",range = "A1:N19",col_names = TRUE)%>% 
# dplyr::select(1:8,10,12:14)%>% 
  mutate(sex=if_else(sex=="Male",1,2))

FAO_child <- read_excel("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient standards/FAO_energyrequirements_2011.xlsx",sheet="Child_TEEPAL",range = "A1:N31",col_names = TRUE)%>% 
# dplyr::select(1:8,10,12:14)%>% 
  mutate(sex=if_else(sex=="Male",1,2))
...
w.I.roster.2011=w.I.roster.2011%>%
  ...
  mutate(kcal_FAO2=multiplier*(kc+kc1*weight+kc2*(weight^2)))%>%
...
mutate(pregnant=if_else(lifestage=="Pregnancy",1,0),lactating=if_else(lifestage=="Lactation",1,0))%>%
       dplyr::select(-(1:2))%>%
  mutate(sex=if_else(is.na(sex),1,sex))

#duplicate baby which is missing sex
babygirl=ref_wtht%>%
  filter(agemin==1)%>%
  mutate(sex=2)

ref_wtht=bind_rows(ref_wtht,babygirl)%>%
  arrange(agemin)
...
rm(DRI_standards,calc_iron,activity2,new,h,hh,base_iom,x,babygirl)
...
allref=w.I.roster.2011%>%
  mutate(refwt_India_g=if_else(weight<refwt_India,1,0),
         refwt_FAO_g=if_else(weight<refwt_FAO,1,0),
         refwt_IOM_g=if_else(weight<refwt_IOM,1,0),
         refht_FAO_g=if_else(height<refht_FAO,1,0),
         refht_IOM_g=if_else(height<refht_IOM,1,0))%>%
  dplyr::select(hhid,mid,age,age_range,sex,weight,contains("wt_"),height,contains("ht_"),AE_India,AE_FAO,AE_IOM,AE_FAO2,AE_IOM2,contains("code"),contains("R1"))
...
w_allref=w_allref%>%
  arrange(age_range)%>%
mutate_at(vars(contains("_g")),funs(scales::percent(.,accuracy=.01)))

#%>%mutate(across(is.numeric, round, 4))

wh_allref=w_allref%>%
  dplyr::select(sex,age_range,age,weight,refwt_India,refwt_India_g,refwt_FAO,refwt_FAO_g,refwt_IOM,refwt_IOM_g,
         height,refht_FAO,refht_FAO_g,refht_IOM,refht_IOM_g)

AE_allref=w_allref%>%
  dplyr::select(sex,age_range,age,contains("AE"),n)
...
whref_print<-kable(wh_allref,booktabs=TRUE,caption="Comparison of Reference Standards for Weight and Height",
      col.names=c("Sex","Age Range","Age","Sample","Reference","% Below","Reference","% Below","Reference","% Below","Sample","Reference","% Below","Reference","% Below"),
                  escape=F)%>%
  kable_styling(full_width=FALSE,"striped")%>%
  add_header_above(c(" "=3," "=1,"India"=2, "WHO/FAO"=2,"IOM"=2," "=1,"WHO/FAO"=2,"IOM"=2)) %>%
add_header_above(c(" "=3,"Weight (kg) "=7, "Height (cm) "=5)) %>%
   column_spec(c(1,3,10,15),border_right = TRUE)%>%
...
AE_print<-kable(AE_allref,booktabs=TRUE,caption="Comparison of Adult Equivalents",
      col.names=c("Sex","Age Range","Age","India","WHO/FAO","IOM","WHO/FAO","IOM","N"),
                  escape=F)%>%
  kable_styling(full_width=FALSE,"striped")%>%
  add_header_above(c(" "=3,"Reference Weight/Height"=3,"Actual Weight/Height"=2," "=1)) %>%
add_header_above(c(" "=3,"Adult Equivalents"=5," "=1)) %>%
   column_spec(c(1,3,8,9),border_right = TRUE)%>%
  #row_spec(c(3:4,7:8,11:12),background="gray90")%>%
 footnote(general="Data are population-weighted means using sample weights provided by IFPRI.
