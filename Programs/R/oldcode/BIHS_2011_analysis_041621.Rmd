...
```
ANOVA:any health worker visits & kcal inadequacy, actual kcal inadequacy FAO group
```{r}
...
ggplot(hh_BMI_workervisits_num,aes(num_visited_sixmo,BMI))+geom_boxplot(aes(group=num_visited_sixmo))+ylim(10,30)+xlab("Number of Health Worker Visits in the past six months")+ylab("Average HH BMI for children under 18")+ geom_smooth(method = "lm", se = FALSE)
...
```
Linear Regression - number of health worker visits & actual kcal inadequacy FAO group
```{r}
...
# task: add an abline
ggplot(hh_kcal_workervisits_num,aes(num_visited_sixmo,actual_inadequate_FAO_grp))+geom_jitter(aes(group=num_visited_sixmo))+xlab("Number of Health Worker Visits in the past six months")+ylab("Percentage of Children with Caloric Inadequcy")+ geom_smooth(method = "lm", se = FALSE)
...
ggplot(hh_hhds_workervisits_num,aes(num_visited_sixmo,as.numeric(factor(hhds))))+geom_jitter(aes(group=num_visited_sixmo))+xlab("Number of Health Worker Visits in the past six months")+ylab("Household Dietary Diversity Score")+ geom_smooth(method = "lm", se = FALSE)
