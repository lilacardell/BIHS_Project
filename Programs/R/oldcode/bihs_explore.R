...
packages  =  c('data.table','tidyverse','haven','stargazer','plm','lubridate','survey','psych','knitr','xtable','tables',
               'MASS','matrixcalc','car','stringr','styler','readxl','magrittr','sjmisc') ## you can add more packages here
lapply(packages, pkgTest)


#setwd("~/Box Sync/BIHS Project")
...
# # 
# # #reorganize this frame by hhid
# o1_f_2015 = o1_f_2015[order(o1_f_2015$a01,o1_f_2015$o1_01),]
# # 
# # #count #hhs in each program
# program_count = aggregate(a01~slno, data=u_m_2015, FUN = length)
# # 
# # #count food gifts
# food_gift = aggregate(a01~o1_01, data=o1_f_2015, FUN = length)




### Import Nutrition Data from Govt of Bangladesh ---------------------------
(nutrients = 
...
# Edit BIHS menu and ingredient codes -------------------------------------
...
listx = dplyr::select(x1_f_2011_m,contains("x1_07"))
namesx = names(listx)
...
x1_f_2011_I = left_join(x1_f_2011_m,ingredientcodes,by=c("x1_07_01"="ingredient")) %>%
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_01"="BIHS_code")) %>%
  ...
  mutate(cal1 = x1_08_01*kcal1*edible1/100) 
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_02"="BIHS_code")) %>%
  ...
  mutate(cal2 = x1_08_02*kcal2*edible2/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_03"="BIHS_code")) %>%
  ...
  mutate(cal3 = x1_08_03*kcal3*edible3/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_04"="BIHS_code")) %>%
  ...
  mutate(cal4 = x1_08_04*kcal4*edible4/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_05"="BIHS_code")) %>%
  ...
  mutate(cal5 = x1_08_05*kcal5*edible5/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_06"="BIHS_code")) %>%
  ...
  mutate(cal6 = x1_08_06*kcal6*edible6/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_07"="BIHS_code")) %>%
  ...
  mutate(cal7 = x1_08_07*kcal7*edible7/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_08"="BIHS_code")) %>%
  ...
  mutate(cal8 = x1_08_08*kcal8*edible8/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_09"="BIHS_code")) %>%
  ...
  mutate(cal9 = x1_08_09*kcal9*edible9/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_10"="BIHS_code")) %>%
  ...
  mutate(cal10 = x1_08_10*kcal10*edible10/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_11"="BIHS_code")) %>%
  ...
  mutate(cal11 = x1_08_11*kcal11*edible11/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_12"="BIHS_code")) %>%
  ...
  mutate(cal12 = x1_08_12*kcal12*edible12/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_13"="BIHS_code")) %>%
  ...
  mutate(cal13 = x1_08_13*kcal13*edible13/100)
...
x1_f_2011_I = left_join(x1_f_2011_I,calories,by=c("x1_07_14"="BIHS_code")) %>%
  ...
  mutate(cal14 = x1_08_14*kcal14*edible14/100)
...
# Integrate X1 (recipes) with X2 (individual consumption) -----------------
...
x2_f_2011_I = left_join(x2_f_2011,calpermeal,by=c("a01","x2_meal"="x1_03","x2_08"="x1_05"))%>%
  dplyr::select(-starts_with("cal"))%>%
  mutate(I_mealcal=mealcalpergram*x2_09)

daycal_2011_I = x2_f_2011_I %>%
  group_by(a01, x2_01) %>%
  summarise(I_daycal=sum(I_mealcal,na.rm=TRUE),mealdata=n())%>%
...
roster_2011_I=left_join(b1_m_2011,w1_f_2011,by=c("a01","mid","sample_type")) %>%
  dplyr::rename(hhid=a01,pregnant=w1_01,lactating=w1_02,
                    ...
                    education=b1_08,occupation=b1_10)
...
roster_2011_I = roster_2011_I %>%
  ...
  mutate(hhsize=n(),
         ...
         otheradult=ifelse(mid>2 & adult==1,1,0),
...
# Create Household Roster and Head and Spouse Rosters---------------------------------------------
...
roster_2011_hh = roster_2011_I %>%
  ...
  summarise(hhsize = n(), adult=sum(adult),child=sum(child), poly=max(poly),
            ...
            mealdata_hhmin=min(mealdata),hhcalperday=sum(I_daycal),sampletype=first(sample_type))
...
roster_2011_head = filter(roster_2011_I,relation==1) %>%
   mutate(agehead=age, genderhead=gender, 
          ...
          BMIhead=BMI,I_daycalhead=I_daycal) %>%
...
roster_2011_spouse = filter(roster_2011_I,relation==2) %>%
            mutate(agespouse=age,genderspouse=gender,
                   ...
                   BMIspouse=BMI,I_daycalspouse=I_daycal)%>%
...
# Calculate village level prices and HH expenditures ----------------------
...
roster_2011_hh=left_join(roster_2011_hh,foodexp_hh,by=c("hhid"="a01")) %>%
  mutate(foodpurch_hhwkpc = foodpurch_hhwk/AE,
         ...
         foodshare = foodpurch_hhwk/nonfoodexp_hhwk,na.rm=TRUE)
...
roster_hh_DST = roster_2011_hh %>%
  filter(genderhead==1 & maritalhead==2 & porl_hh==0 & spouseabroad_hh==0 & abroadhead==0 & poly==0) 
...
# 
# x= dplyr::bind_cols(roster_hh.summary,roster_hh_DST.summary) %>%
#   dplyr::select(-rowname1)%>%
#   dplyr::rename(obs_full=n,mean_full=mean,sd_full=sd,obs_sample=n1,mean_sample=mean1,sd_sample=sd1) %>%
#   dplyr::filter(rowname=="nonfoodexp_hhwkpc" | rowname=="foodpurch_hhwkpc" | rowname=="hhsize" | rowname=="AE" | rowname=="boy" | rowname=="girl" | rowname=="otheradult" 
...
# 
# 
# kable(x,"html",booktabs=TRUE,digits=2,caption="Table 1", col.names=c("Variable","N","Mean","SD","N","Mean","SD"),align='l')%>%
#   kable_styling(full_width=TRUE)%>%
#   pack_rows("Household Head",8,12)%>%
#   pack_rows("Spouse",13,17)%>%
#   add_header_above(c(" "=1,"All Households"=3,"Sample"=3))





# Import Nutrition Benchmarks ---------------------------------------------
#mder US
#mder bangladesh/india
