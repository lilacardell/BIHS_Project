...
packages  =  c('data.table','tidyverse','haven','stargazer','plm','lubridate','survey','psych','knitr','xtable','tables','kableExtra','fuzzyjoin','MASS','matrixcalc','car','stringr','styler','readxl','magrittr','sjmisc','readxl','xlsx','srvyr','gdata') ## you can add more packages here
...
setwd("C:/Users/lilac2/Box/BIHS Project/Data/Clean Data/nutrient tables")
...
recipe_med=recipe_sum%>%
  group_by(menucode)%>%
   filter(kcal>0)%>%
  dplyr::summarise(med_kcal=mean(kcal/iweight,na.rm=TRUE),sd_kcal=sd(kcal/iweight,na.rm=TRUE),
                   med_protein=mean(protein/iweight,na.rm=TRUE),sd_protein=sd(protein/iweight,na.rm=TRUE),
                   med_fat=mean(fat/iweight,na.rm=TRUE),sd_fat=sd(fat/iweight,na.rm=TRUE),
                   med_carb=mean(carb/iweight,na.rm=TRUE),sd_carb=sd(carb/iweight,na.rm=TRUE),
                   med_calcium=mean(calcium/iweight,na.rm=TRUE),sd_calcium=sd(calcium/iweight,na.rm=TRUE),
                   med_iron=mean(iron/iweight,na.rm=TRUE),sd_iron=sd(iron/iweight,na.rm=TRUE))%>%
  filter_all(all_vars(!is.na(.)))


for (x in nutnames){
 recipe_med[[paste("min",x,sep="_")]]<-recipe_med[[paste("med",x,sep="_")]]-recipe_med[[paste("sd",x,sep="_")]]
 recipe_med[[paste("max",x,sep="_")]]<-recipe_med[[paste("med",x,sep="_")]]+recipe_med[[paste("sd",x,sep="_")]]
}
...
for (x in nutnames){
recipe_hh[[x]]=if_else(recipe_hh[[x]]==0|is.nan(recipe_hh[[x]])|is.na(recipe_hh[[x]])|
                       recipe_hh[[x]]>recipe_hh[[paste("max",x,sep="_")]]|recipe_hh[[x]]<recipe_hh[[paste("min",x,sep="_")]],
                       recipe_hh[[paste("med",x,sep="_")]]*recipe_hh$iweight,recipe_hh[[x]])
}
...
rm(recipe_med,recipe_sum,recipes,menucodes,nutrients,missing,x1_female_2011,x1_f_2011_m,x,menu_items,ingredientcodes,missing_ingredients,missing_recipes)
```
...
nrow(filter(nutrition_2011_I,I_kcal==0|is.na(I_kcal))) #91 without calorie info
...
w.I.roster.2011=left_join(roster_2011_I,weights_2011,by=c("hhid"="a01"))%>%
  filter(sampletype==2 |sampletype==3)%>%
  filter(I_kcal>0 & mealdata>0 &poly==0 & age>=2&abroad==2&spouseabroad==0)
...
listx = dplyr::select(w.hh.roster.2011,hhsize,adult,child,totexp_hhwk,foodshare,hhds,contains("head"),contains("spouse"),-contains("abroad"),-contains("sex"),-contains("marital"))
...
ab$rownames<-c("Size","Adults","Children","Total Weekly Expenditure (2012 Taka)","Food Share of Total Expenditure","Household Dietary Diversity Score","Age","Literacy Status","No Schooling","Secondary Schooling","Agricultural Worker","Body Mass Index (BMI)","Reported Calorie Consumption","Age","Literacy Status","No Schooling","Secondary Schooling","Agricultural Worker","Body Mass Index (BMI)","Reported Calorie Consumption")
ab=ab%>%
  dplyr::select(rownames,mean,SE)

setwd("C:/Users/lilac2/Box/BIHS Project/Programs/R/output")

descriptives=kable(ab,booktabs=TRUE,digits=2,
...
w.I.roster.2011=fuzzy_left_join(w.I.roster.2011,AE_India, 
                ...
                match_fun=list(`>=`, `<=`))%>%
  mutate(AE_India=if_else(sex==1,AE_male,if_else(sex==2 & porl==1,1.71,AE_female)))%>%
  dplyr::select(-(39:42))
...
w.I.roster.2011=w.I.roster.2011%>%
  ...
  mutate(range_India=if_else(!is.na(agemin),paste(as.character(agemin),as.character(agemax),sep="-"),as.character(NA)),
    kcal_India=if_else(pregnant==1,kcal_India+350,if_else(lactating==1,kcal_India+560,kcal_India)),
         protein_India=if_else(pregnant==1,protein_India+23,if_else(lactating==1,protein_India+16,protein_India)),
    calcium_India=if_else(porl==1,1200,calcium_India),
    iron_India=if_else(porl==1,21,iron_India))%>%
  dplyr::select(-contains(".y"),-agemin,-agemax)



#FAO requirements and AE
FAO_adult <- read_excel("./Data/Clean Data/nutrient standards/FAO_energyrequirements_2011.xlsx",sheet="Adult_BMRPAL",range = "A1:N19",col_names = TRUE)%>% 
 dplyr::select(1:3,7,8,10,12:14)%>% 
  mutate(sex=if_else(sex=="Male",1,2))

FAO_child <- read_excel("./Data/Clean Data/nutrient standards/FAO_energyrequirements_2011.xlsx",sheet="Child_TEEPAL",range = "A1:N31",col_names = TRUE)%>% 
 dplyr::select(1:3,7,8,10,12:14)%>% 
  mutate(sex=if_else(sex=="Male",1,2))
...
#calculate BMR and FAO daily energy requirement=BMR x PAL, add 285 kcal/d for pregnancy, and 460-500 kcal/day if breastfeeding
w.I.roster.2011=w.I.roster.2011%>%
 mutate(sex=as.numeric(sex),activityI=if_else(age<14,"light",activity))%>%
  fuzzy_left_join(FAO,by=c("sex","activityI"="activity","age"="agemin","age"="agemax"),match_fun=list(`==`,`==`,`>=`,`<`))%>%
...
mutate(range_FAO=if_else(!is.na(agemin),paste(as.character(agemin),as.character(agemax),sep="-"),as.character(NA)),
       ...
       protein_FAO=if_else(pregnant==1,protein_FAO+15.75,if_else(lactating==1,protein_FAO+13.67,protein_FAO)))%>%
  dplyr::select(-contains(".y"),-EER,-agemin,-agemax)

#create individual energy requirements
w.I.roster.2011=w.I.roster.2011%>%
    mutate(PAL=if_else(activity=="light",1.4,if_else(activity=="moderate",1.7,                             if_else(activity=="heavy",2,as.numeric(NA)))))%>%
  mutate(kcal_FAO2=if_else(age<18&sex==1,310.2+63.3*weight-0.263*(weight^2),
                   if_else(age<18&sex==2,263.4+65.3*weight-0.454*(weight^2),
                    if_else(age>=18&age<30&sex==1,15.057*weight+692.2,
                   if_else(age>=30&age<60&sex==1,11.472*weight+873.1,
                    if_else(age>=60&sex==1,11.711*weight+587.7,
                    if_else(age>=18&age<30&sex==2,14.818*weight+486.6,
                   if_else(age>=30&age<60&sex==2,8.126*weight+845.6,
                      if_else(age>=60&sex==2,9.082*weight+658.5,as.numeric(NA))))))))))%>%
  mutate(kcal_FAO2=if_else(age>=18,kcal_FAO2*PAL,if_else(age>=14&age<18&activity=="heavy",kcal_FAO2*1.15,if_else(age>=14&age<18&activity=="light",kcal_FAO2*0.85,if_else(age<14,kcal_FAO2*0.85,kcal_FAO2)))))%>%
  mutate(kcal_FAO2=if_else(pregnant==1,kcal_FAO2+285,if_else(lactating==1,kcal_FAO2+485,kcal_FAO2)))%>%
  mutate(protein_FAO2=if_else(age>=18,weight*0.66,weight*0.72))%>%
  mutate(protein_FAO2=if_else(pregnant==1,protein_FAO2+15.75,
                              if_else(lactating==1,protein_FAO2+13.67,protein_FAO2)))%>%
  mutate(AE_FAO2=kcal_FAO2/2886.1)

library(plyr)

#DRI standards from Schenider/Hertforth
DRI_standards = read_dta("./Programs/Stata/EAR+UL+AMDR+CDDR_allgroups.dta")%>%
...
dplyr::mutate(sex=if_else(sex==0,1,2),activity=if_else(pal%in%c(1,2),"light",if_else(pal==3,"moderate",if_else(pal==4,"heavy",as.character(NA)))),
...
ref_wtht = read_excel("./Programs/Stata/Supp2_WHO_growth_reference_tables_2006.xlsx",sheet="Weights Heights by DRI Group",range = "A1:I26",col_names = TRUE)%>% 
  dplyr::rename(agemin=4,agemax=5,refwt_FAO=6,refwt_IOM=7,refht_FAO=8,refht_IOM=9)%>%
  mutate(sex=if_else(sex=="Male",1,2),agemin=if_else(agemax==3,2,agemin),)%>%
    filter(agemin>=2)%>%
  mutate(agemax=as.numeric(agemax),pregnant=0,lactating=0)%>%
mutate(agemin=if_else(lifestage=="Older Adult",70,agemin),agemax=if_else(lifestage=="Older Adult",150,agemax))%>%
mutate(pregnant=if_else(lifestage=="Pregnancy",1,0),lactating=if_else(lifestage=="Lactation",1,0))%>%
       dplyr::select(-(1:2))
...
w.I.roster.2011=w.I.roster.2011%>%
  mutate(pregnant=if_else(pregnant==1&lactating==1,0,pregnant))%>%
...
#generate individual specific standards for IOM
w.I.roster.2011=w.I.roster.2011%>%
  mutate(PAL=if_else(activity=="light",1,if_else(activity=="moderate",1.2,                     if_else(activity=="heavy",1.45,as.numeric(NA)))))%>%
  mutate(kcal_IOM2=if_else(sex==1&age>=19,662-(9.53*age)+PAL*((1.591*weight)+(539.6*height/100)),if_else(sex==2&age>=19,354-(6.91*age)+PAL*((9.36*weight)+(726*height/100)),
          if_else(sex==1&age>=9&age<=18,88.5-(61.9*age)+PAL*((26.7*weight)+(903*height/100))+25,
          if_else(sex==2&age>=9&age<=18,135.3-(30.8*age)+PAL*((10*weight)+(934*height/100))+25,
           if_else(sex==1&age>=3&age<=8,88.5-(61.9*age)+PAL*((26.7*weight)+(903*height/100))+20,
            if_else(sex==2&age>=3&age<=8,135.3-(30.8*age)+PAL*((10*weight)+(934*height/100))+20,
                    if_else(age>=2&age<3,(89*weight-100)+20,as.numeric(NA)))))))))%>%            mutate(kcal_IOM2=if_else(sex==2&pregnant==1,kcal_IOM2+340,
                          if_else(sex==2&lactating==1,kcal_IOM2+500-170,kcal_IOM2)))%>%
  mutate(protein_IOM2=if_else(age>=2&age<4,weight*0.87,
                        if_else(age>=4&age<9,weight*0.76,
                        if_else(age>=9&age<14,weight*0.76,
                         if_else(age>=14&age<=18&sex==1,weight*0.73,
                          if_else(age>=14&age<=18&sex==2,weight*0.71,
                                  if_else(age>=19,weight*0.66,as.numeric(NA))))))))%>%
  mutate(protein_IOM2=if_else(sex==2&pregnant==1,weight*0.88,
                              if_else(sex==2&lactating==1,weight*1.05,protein_IOM2)))%>%
  mutate(protein_amdr2=if_else(age<4,(.05*kcal_IOM2)/4,(.1*kcal_IOM2)/4),
         fat_amdr2=if_else(age<4,(.3*kcal_IOM2)/9,
                            if_else(age>=4&age<=18,(.25*kcal_IOM2)/9,(0.2*kcal_IOM2)/9)),
         carb_amdr2=(0.45*kcal_IOM2)/4)%>%
  mutate(AE_IOM2=kcal_IOM2/x)
...
hh=w.I.roster.2011%>%
  ...
  dplyr::rename_at(.vars=vars(2:6),.funs=funs(sub("AE_","hhsize_",.)))
    #hhsize_India=sum(AE_India,na.rm=TRUE),hhsize_FAO=sum(AE_FAO,na.rm=TRUE),hhsize_IOM=sum(AE_IOM,na.rm=TRUE,))
...
w.I.roster.2011=left_join(w.I.roster.2011,h,by="hhid")%>%
  mutate(share_India=AE_India/hhsize_India,share_FAO=AE_FAO/hhsize_FAO,share_IOM=AE_IOM/hhsize_IOM,
         share_FAO2=AE_FAO2/hhsize_FAO2,share_IOM2=AE_IOM2/hhsize_IOM2,
         age_range=if_else(age>=1&age<3,"infant (2-3)", if_else(age>=3&age<10,"child (3-10)", if_else(age>=10&age<18,"adolescent (10-18)",if_else(age>=18&age<30,"young adult (18-30)",if_else(age>=30&age<=60,"adult (30-60)",if_else(age>60,"senior (60+)",as.character(NA))))))))%>%
  fuzzy_left_join(ref_wtht,by=c("sex","age"="agemin", "age"="agemax","pregnant","lactating"),match_fun=list(`==`,`>=`, `<`,`==`,`==`))%>%
...
allref=w.I.roster.2011%>%
  dplyr::select(hhid,mid,age,age_range,sex,weight,contains("wt_"),height,contains("ht_"),AE_India,AE_FAO,AE_IOM,AE_FAO2,AE_IOM2,34:38,-aeu)
...
w_allref=allref%>%
  ...
  dplyr::select(-(33:40),-contains("hhid"),-contains("mid"),-contains("_se"))
...
age=c("infant (2-3)","child (3-10)","adolescent (10-18)","young adult (18-30)","adult (30-60)","senior (60+)")
...
#%>%
# mutate_at(vars(contains("AE")),funs(scales::percent(.,accuracy=.01)))
...
allref_print<-kable(w_allref,booktabs=TRUE,caption="Comparison of Reference Standards",
      col.names=c("Sex","Age Range","Age","Sample","India","WHO/FAO","IOM","Sample","WHO/FAO","IOM",
                  "India","WHO/FAO","IOM","WHO/FAO","IOM","N"),
                  escape=F)%>%
  kable_styling(full_width=FALSE,"striped")%>%
  add_header_above(c(" "=3," "=1,"Reference Standard"=3," "=1,"Reference Standard"=2,"Reference Weight/Height"=3,"Actual Weight/Height"=2," "=1)) %>%
add_header_above(c(" "=3,"Weight (kg) "=4, "Height (cm) "=3,"Adult Equivalents"=5," "=1)) %>%
   column_spec(c(1,3,7,10,15,16),border_right = TRUE)%>%
...
save_kable(allref_print,"refwtht.png")
print(allref_print)

#rm(allref)

#allocate share of hh nutrient total
ref=c("India","FAO","IOM","FAO2","IOM2")
for (i in 1:length(nutnames)) {
  
  temp=w.I.roster.2011%>%
    dplyr::select(1:4,BMI,age_range,contains("share"),contains(nutnames[[i]]),hhds,34:38)
...
#identify inadequacy for each nutrient
...
kcal_inadequacy=kcal_inadequacy%>%
  mutate(BMI=if_else(age>=18,BMI,as.numeric(NA)),
         BMI_inadequate=if_else(BMI<21.5,1,0),
         BMI_gap=if_else(BMI_inadequate==1,(BMI-21.5)/18.5,as.numeric(NA)))
...
kcal_print<-kable(w_kcal,booktabs=TRUE,caption="Incidence of Energy Inadequacy",
      col.names=c("Sex","Age Range","BMI",rep(list("India","FAO/WHO","IOM","FAO/WHO","IOM"),2),
                  "BMI",rep(list("India","FAO/WHO","IOM","FAO/WHO","IOM"),2)),
      escape=F)%>%
  kable_styling(full_width=FALSE,"striped","bordered")%>% 
    add_header_above(c(" "=2," " =1,rep(list("wrt Age Group Req"=3,"wrt Individual Req"=2),2),
                       " " =1,rep(list("wrt Age Group Req"=3,"wrt Individual Req"=2),2)))%>% 
  add_header_above(c(" "=2,rep(list(" "=1,"Reported Individual Consumption"=5,"Allocated Household Consumption"=5),2)))%>% 
add_header_above(c(" "=2,"Frequency of Inadequacy"=11, "Intensity of Inadequacy"=11))%>% 
   column_spec(c(2,3,8,13,14,19,24),border_right = TRUE)%>%
  row_spec(c(3:4,7:8,11:12),background="gray90")%>%
 footnote(general="Data are population-weighted means using sample weights provided by IFPRI.
...
save_kable(kcal_print,"kcal.png")
print(kcal_print)
...
#India standards
India_inadequacy=w.I.roster.2011%>%
...
#FAO standards
FAO_inadequacy=w.I.roster.2011%>%
...
#IOM standards
IOM_inadequacy=w.I.roster.2011%>%
 ...
 dplyr::select(vcode,dvcode,popweightR1,sex,range_IOM,contains("kcal"),contains("protein"),contains("carb"),contains("fat"),contains("calcium"),contains("iron"),-contains("upper"),-protein_IOM)
